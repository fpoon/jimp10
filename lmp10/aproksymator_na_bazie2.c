...
#define TESTBASE 500
	...
	for (j = 0; j < nb; j++) {
		for (i = 0; i < nb; i++)
			for (k = 0; k < pts->n; k++)
				add_to_entry_matrix(eqs, j, i, dfi(a, b, nb, i, x[k]) * legendre(x[k],i));/*fi(a, b, nb, j, x[k]));*/
	...
	if (alloc_spl(spl, nb) == 0) {
		for (i = 0; i < spl->n; i++) {
			...
			for (k = 0; k < nb; k++) {
				...
				spl->f[i]  += ck * legendre(xx,nb);//fi  (a, b, nb, k, xx);
				spl->f1[i] += ck * legendre(xx,nb);//dfi (a, b, nb, k, xx);
				spl->f2[i] += ck * legendre(xx,nb);//d2fi(a, b, nb, k, xx);
				spl->f3[i] += ck * legendre(xx,nb);//d3fi(a, b, nb, k, xx);
			}
		}
	}
